...
class DateModifier
{
    public function calculateDayDifference(string $dateFrom, string $dateTo): int
    {
        ...
        $days += $this->getDaysFromBeginning($dateToParts[0], $dateToParts[1], $dateToParts[2]) - $this->getDaysFromBeginning($dateFromParts[0], $dateFromParts[1], $dateFromParts[2]);
        return $days;
    }
    ...
    private function getDaysFromBeginning(int $year, int $month, int $days): int
    {
        ...
        $halfMonths = $month - $fullMonths;

        $days = $fullMonths * 31 + $halfMonths * 30;
        $days--;
